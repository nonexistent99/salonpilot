...
export async function executeCustomerTool(args: ExecuteToolArgs) {
  ...
  try {
    switch (args.toolName) {
      case "listServices": {
        const rows = await sql(
          `SELECT s.id, s.name, s.description, s.price, s.duration_minutes,
           ...
           LEFT JOIN professionals p ON p.id = ps.professional_id AND p.salon_id = s.salon_id AND p.active = TRUE
           WHERE s.salon_id = $1
             AND s.active = TRUE
      ...
      case "getServiceDetails": {
        const service = await sqlOne(
          ...
          [
            args.salonId,
            asUuid(parsed.service_id),
            parsed.service_name || parsed.service_id || null,
          ],
        );
        result = service
          ? { success: true, service }
          : { success: false, message: "Servico nao encontrado." };
        break;
      }
      ...
      case "cancelAppointmentRequest": {
        ...
        result = {
          ...
          message: "Uma atendente vai assumir a conversa.",
        };
        break;
      }
      ...
      default:
        ...
        result = {
          success: false,
          message: `Tool desconhecida: ${args.toolName}`,
        };
    }
  } catch (error) {
    status = "error";
    result = {
      success: false,
      message: error instanceof Error ? error.message : "Erro na tool.",
    };
  }
