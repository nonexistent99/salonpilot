  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-3 right-3 w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-secondary"
        >
          <X className="w-4 h-4" />
        </button>
              ...
              <div className="max-h-40 overflow-y-auto rounded-lg border border-border bg-card/50">
                {filteredLeads.length === 0 ? (
                  <p className="p-3 text-xs text-muted-foreground text-center">
                    {leads?.length === 0
                      ? "Nenhum lead cadastrado. Busque leads primeiro."
                      : "Nenhum resultado."}
                  </p>
                ) : (
...
export function AgendaSection() {
  ...
  const meetingsByDate = (monthMeetings || []).reduce<Record<string, number>>(
    (acc, m) => {
  ...
  const scheduledThisMonth = (monthMeetings || []).filter(
                  ...
                  >
                    ...
                    {meetingCount > 0 && (
                      <div className="flex gap-0.5 mt-0.5">
                        {Array.from({ length: Math.min(meetingCount, 3) }).map(
                          (_, i) => (
                            <div
                              key={i}
                              className={cn(
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  ...
                  {selectedMeetings.length === 1 ? "reunião" : "reuniões"}
                </p>
              </div>
              <button
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          {(meeting.phone || meeting.leads?.phone) && (
                            <a
                              href={`tel:${meeting.phone || meeting.leads?.phone}`}
                              className="h-7 px-2.5 rounded-md bg-primary/10 text-primary text-xs hover:bg-primary/20 transition-colors flex items-center gap-1"
                            >
                          ...
                          {meeting.status === "scheduled" && (
                            <button
                              onClick={() =>
                                handleUpdateStatus(meeting.id, "completed")
                              }
                          ...
                          {meeting.status === "scheduled" && (
                            <button
                              onClick={() =>
                                handleUpdateStatus(meeting.id, "no_show")
                              }
